...
class SearchPage extends StatelessWidget {
  ...
  Function(Result) _navCallBack(BuildContext context) =>
      (Result selectedResult) {
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => DetailPage(selectedResult)));
      };
                  ...
                  builder: (context, snapshot) {
                    if (!snapshot.hasData)
                      ...
                      return ResultListWidget(
                          snapshot.data.results, _navCallBack(context));
                    else
                      return CustomErrorWidget(snapshot.data.error);
                  })
              : CustomErrorWidget(
                  'Type atleast 3 or more characters to search!')));
}
